...
def get_trained_model(data, data_schema, hyper_params):
    ...
    preprocess_pipe = get_preprocess_pipeline(train_data, None, data_schema)
...
def train_model(train_data, data_schema, preprocess, hyper_params):
    info = data_schema["inputDatasets"]["binaryClassificationBaseMainInput"]
    target_class = info["targetField"]
    ...
    classifier = Classifier(preprocess, id_field, target_class, **hyper_params)
    model = classifier.fit(train_data, _categorical, _numerical)
    return model


def get_preprocess_pipeline(train_data, valid_data, data_schema):
